Contributing to Mangacore
=======

We're working hard to make *mangacore* the most powerful JavaScript library for working with mangacoin. Our goal is to have *mangacore* be a library that can be used by anyone interested in mangacoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in mangacore that any change must adhere.
...
When possible, mangacore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Mangacore methods. For example, calling `JSON.stringify` on a Mangacore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'mangacore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the mangacore object.
...
```sh
git remote add mangacoinproject git@github.com:mangacoinproject/mangacore.git
git pull --rebase mangacoinproject master
```
...
```
Finally go to [github.com/mangacoinproject/mangacore](https://github.com/mangacoinproject/mangacore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of mangacore.

If you have any questions feel free to post them to
[github.com/mangacoinproject/mangacore/issues](https://github.com/mangacoinproject/mangacore/issues).
